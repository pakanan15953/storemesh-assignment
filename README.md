...
## Architectural Choices (การตัดสินใจด้านสถาปัตยกรรม)

การออกแบบระบบและเลือกเครื่องมืออิงตามหลักการพัฒนาซอฟต์แวร์ระดับสากลและความต้องการของเกณฑ์ประเมิน:

1. **Decoupled Architecture (สถาปัตยกรรมแบบแยกส่วน):** 
   * ทำการแยกส่วนติดต่อผู้ใช้งาน (React Frontend) ออกจากระบบประมวลผลหลัก (Django REST API) อย่างชัดเจน (Separation of Concerns) เพื่อให้การพัฒนาและการทดสอบโค้ดทั้งสองฝั่งเป็นอิสระต่อกัน และง่ายต่อการปรับขนาดระบบ (Scalability) หรือการเปลี่ยนเทคโนโลยีในอนาคต
2. **Relational Database (PostgreSQL):** 
   * เลือกใช้ PostgreSQL เป็นฐานข้อมูลเชิงสัมพันธ์ที่รองรับคุณสมบัติ ACID Compliance ซึ่งจำเป็นอย่างยิ่งสำหรับโปรเจกต์ประเภท E-commerce ที่มีความเปราะบางสูงในเรื่องของความถูกต้องของข้อมูลทางการเงิน รายการธุรกรรม และสต็อกสินค้า
3. **Database Integrity & Transaction Control (Atomic Transactions):**
   * ระบบสั่งซื้อ (Checkout) และหักสต็อกใช้กลไก `transaction.atomic()` ของ Django ควบคุม เพื่อให้มั่นใจว่าข้อมูลทุกอย่างจะถูกบันทึกอย่างครบถ้วน หรือยกเลิกทั้งหมด (Rollback) หากสินค้าชิ้นใดชิ้นหนึ่งสต็อกไม่พอ ป้องกันไม่ให้ฐานข้อมูลตกอยู่ในสถานะที่ข้อมูลไม่ตรงกัน (Inconsistent state)
4. **Stateless Security (JWT Authentication):**
   * เลือกใช้ JSON Web Tokens (SimpleJWT) ในการระบุตัวตนของผู้ใช้ เพื่อหลีกเลี่ยงการเก็บ Session บนเซิร์ฟเวอร์หลังบ้าน ทำให้สเกลการทำงานของเซิร์ฟเวอร์ทำได้ง่าย และรองรับการทำสิทธิ์การเข้าถึง API ตามบทบาทหน้าที่ (Role-based Access Control - RBAC)
5. **Environment Consistency (Docker Containerization):**
   * รวบรวมคอนฟิกบริการทั้งหมด (Database, Python/Django, Node.js/React) ให้อยู่ในรูปของตู้คอนเทนเนอร์ เพื่อขจัดปัญหา "รันได้ในเครื่องฉัน แต่เปิดไม่ได้บนเครื่องคนอื่น" (Eliminating "works on my machine" issues) และตั้งค่าให้มีการรันสคริปต์รอฐานข้อมูล (Wait script) เพื่อเพิ่มความเสถียรในการเริ่มระบบครั้งแรก
